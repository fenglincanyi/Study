...
class FreelineBuildCommand(AbstractCommand):
    ...
    def execute(self):
        file_changed_dict = self._scan_command.execute()# 拿到文件改变信息 json

        if self._dispatch_policy.is_need_clean_build(self._config, file_changed_dict):
            self._setup_clean_builder(file_changed_dict) # 文件需要clean，则设置相关命令 build_command
        ...
        self._build_command.execute()# 根据相应的情况，去分别执行各自的命令 (CleanBuildCommand 或 IncrementalBuildCommand)
