...
class GradleCleanBuilder(CleanBuilder):
    ...
    def generate_sorted_build_tasks(self):
        ...
        install_task = InstallApkTask(self._adb, self._config, wait_for_debugger=self._wait_for_debugger)# 会清理下面的缓存文件
...
class GradleCleanBuildTask(CleanBuildTask):
    ...
    def execute(self):
        ...
        command = self._config['build_script'] # "build_script": "./gradlew :app:assembleOnlineDebug"
        ...
        if 'auto_dependency' in self._config and not self._config['auto_dependency']:
            command += ' -PdisableAutoDependency=true' # 同上 -P 后面的是参数
