...
class Dispatcher(object):# 用来分发执行相关的命令
    ...
    def call_command(self, args):# 分发规则的函数
        self._config = read_freeline_config() # 后续的 self._config 就是读取的 项目描述文件里的内容
        self._args = args
...
def read_freeline_config(config_path=None):# 读取项目描述文件
    if not config_path:
        config_path = os.path.join(os.getcwd(), 'freeline_project_description.json')

    if os.path.isfile(config_path):# 加载文件内容json放到 config 字典中
