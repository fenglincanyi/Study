...
class CoreWorker(threading.Thread): # 继承 Thread
    def __init__(self, workers_num=6):# 默认开启6个线程
    ...
    def add_task(self, task):
        self.queue.put(task) # 把 task 放进队列

    def run(self):# 多个worker 都启动一个线程执行
...
class Worker(threading.Thread):
    ...
    def run(self):
        while True:
            try:
                task = self.queue.get()
                if isinstance(task, ExecutableTask): # 相当于 instanceOf
                    task.execute()
