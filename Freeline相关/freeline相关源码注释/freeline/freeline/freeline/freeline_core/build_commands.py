...
class CleanBuildCommand(AbstractBuildCommand):
    ...
    def execute(self):# 挨个执行 command_list 中的 每个 命令
        map(lambda command: command.execute(), self.command_list)  # map(function, sequence) ：对sequence中的item依次执行function(item)

    def _setup(self):# 添加一系列命令，进行执行
