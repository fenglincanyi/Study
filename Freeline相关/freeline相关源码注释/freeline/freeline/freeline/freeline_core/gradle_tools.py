...
class GradleScanChangedFilesCommand(ScanChangedFilesCommand):
    ...
    def execute(self):
        cache_path = os.path.join(self._config['build_cache_dir'], 'stat_cache.json')# 位置：build/freeline/stat_cache.json
        ...
        build_info = self._get_build_info(self._stat_cache_md5)# 返回上次build时间，和项目根配置文件变化信息

        for module_name, module_info in self.project_info.iteritems():
            if module_name in self._stat_cache:# 对每个module，记录文件的变更，以字典的形式存储
        ...
        # 返回整个项目的文件修改信息，md5信息。 FreelineBuildCommand 的 execute() 方法中
    ...
    def _scan_module_changes(self, module_name, module_path):
        ...
        # scan bulid.gradle
        config_path = os.path.join(module_path, 'build.gradle') # 比较变化的策略也各有不同，分别从文件大小，md5来判断文件是否变化
        if self.__check_changes(module_name, config_path, module_cache, should_check_size=False, should_check_md5=True):
            self._changed_files[module_name]['config'].append(config_path) # 如果变化了，就加到相关字段的 list 中
    ...
    def __check_changes_by_md5(self, fpath):# 读取 cache 就是读取 stat_cache_md5.json文件
        if fpath in self._stat_cache_md5 and get_md5(fpath) == self._stat_cache_md5[fpath]:# 读取stat_cache_md5的 fpath 字段值
            return False
        return True
...
class GradleDispatchPolicy(DispatchPolicy): # 任务分发规则
    def __init__(self):
        DispatchPolicy.__init__(self)

    def is_need_clean_build(self, config, file_changed_dict):# 需要 clean 的规则
